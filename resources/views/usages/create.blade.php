    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex justify-between items-end mb-8 border-b border-neutral-800 pb-6">
            <h2 class="text-4xl md:text-6xl font-bold tracking-tighter text-white">
                Новая запись<span class="text-lime-400">.</span>
            </h2>
        ...
        <div class="border border-neutral-800 p-8">
            <form method="POST" action="{{ route('usages.store') }}" class="space-y-8">
                @csrf
                    ...
                    <select id="thing_id"
                        ...
                        @foreach($things as $thing)
                            <option value="{{ $thing->id }}" 
                                    {{ old('thing_id', request('thing_id')) == $thing->id ? 'selected' : '' }}>
                    ...
                    <select id="place_id"
                        ...
                        @foreach($places as $place)
                            <option value="{{ $place->id }}" 
                                    {{ old('place_id', request('place_id')) == $place->id ? 'selected' : '' }}
                ...
                <div>
                    ...
                    <p class="text-neutral-500 text-xs mt-2">
                        Укажите количество используемых единиц
                    </p>
                </div>
